...
public class InstructionMsgEvent {
    ...
    public int keyAdd(String prefix) {
        ...
        if (reg.matcher(this.ins).find()) {
            ...
            MessageChain repFormat = MessageChain.deserializeFromMiraiCode(rep.replaceAll("\\[vacode:", "[☢:").replaceAll("\\[vacode:", "[☢:").replaceAll(":chatgpt:.*?]", ":chatgpt:***]"), group);

            if (keyword.getContain().contains(type)) {
                keyFormat = MessageChain.deserializeFromMiraiCode(".*?" + key.replaceAll("\\[vacode:", "[☢:").replaceAll("\\[vacode:", "[☢:").replaceAll(":chatgpt:.*?]", ":chatgpt:***]") + ".*?", group);
            ...
            ForwardMessageBuilder forwardMessageBuilder = new ForwardMessageBuilder(group)
                    ...
                    .add(bot, new PlainText(VanillaUtils.enVanillaCodeKey(key).replaceAll("\\[vacode:", "[☢:").replaceAll("\\[vacode:", "[☢:").replaceAll(":chatgpt:.*?]", ":chatgpt:***]")))
                    .add(bot, new PlainText("回复内容文本:"))
                    .add(bot, new PlainText(VanillaUtils.enVanillaCodeRep(rep).replaceAll("\\[vacode:", "[☢:").replaceAll("\\[vacode:", "[☢:").replaceAll(":chatgpt:.*?]", ":chatgpt:***]")));
            boolean tf = false;
            for (long groupId : groups) {
        ...
        if (reg.matcher(this.ins).find()) {
            ...
            if (keywordByPage.size() > 0) {
                for (KeyData keyData : keywordByPage) {
                    ...
                    forwardMessageBuilder.add(bot, MessageChain.deserializeFromMiraiCode(keyData.getWordDecode().replaceAll("\\[vacode:", "[☢:").replaceAll("\\[vacode:", "[☢:").replaceAll(":chatgpt:.*?]", ":chatgpt:***]"), group));
                    forwardMessageBuilder.add(bot, new PlainText("关键词回复:"));
                    forwardMessageBuilder.add(bot, MessageChain.deserializeFromMiraiCode(keyData.getRepDecode(true).replaceAll("\\[vacode:", "[☢:").replaceAll("\\[vacode:", "[☢:").replaceAll(":chatgpt:.*?]", ":chatgpt:***]"), group));
                }
            }
            Api.sendMessage(group, forwardMessageBuilder.build());
            return RETURN_BREAK_TRUE;
